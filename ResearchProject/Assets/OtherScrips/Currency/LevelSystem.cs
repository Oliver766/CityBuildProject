...
public class LevelSystem : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        EventManager.Instance.AddListener<LevelChangedGamerEvent>(onLevelChnaged);
    ...
    private void Initialized()
    {
        try
        {
            ...
            for(int i =1; i < lines.Length - 1; i++)
            {
                ...
                int.TryParse(collumns[0], out lvl);
                int.TryParse(collumns[1], out xp);
                int.TryParse(collumns[2], out curr1);
                int.TryParse(collumns[3], out curr2);
    ...
    private void OnXPAdded(XPAddedGameEvent info)
    {
        ...
        {
            Level++;
            LevelChangedGamerEvent levelChange = new LevelChangedGamerEvent(Level);
            EventManager.Instance.QueueEvent(levelChange);
        }
    }

    private void onLevelChnaged(LevelChangedGamerEvent info)
    {
        XPNow -= xpToNext;
        ...
        CurrencyChangeEvent changeEvent =
            new CurrencyChangeEvent(lvlReward[info.newLvl][0], CurrencyType.Coins);
        EventManager.Instance.QueueEvent(changeEvent);
        changeEvent =
            new CurrencyChangeEvent(lvlReward[info.newLvl][1], CurrencyType.Coins);
        EventManager.Instance.QueueEvent(changeEvent);
    }
